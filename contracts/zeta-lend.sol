...
contract LendingPool is Ownable, Pausable, Constants, ERC20, zContract {
    ...
    function onCrossChainCall(
        ...
        (address token_addr, uint256 amount_token) = abi.decode(
            message,
            (address, uint256)
        );
        token = token_addr;
        amount_tok = amount_token;
        uint8 action = chainID == BITCOIN
            ? uint8(message[0])
            : abi.decode(message, (uint8));

        if (action == 1) {
            supply(token, amount_tok);
        } else if (action == 2) {
            borrow(token, amount_tok);
        } else if (action == 3) {
            repay(token, amount_tok);
        } else if (action == 4) {
            withdraw(token, amount_tok);
        } else {
